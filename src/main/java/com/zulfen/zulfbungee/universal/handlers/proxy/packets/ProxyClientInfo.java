...
public class ProxyClientInfo<P, T, C> extends PacketHandler<P, T, C> {

    public ProxyClientInfo(PacketHandlerManager<P, T, C> packetHandlerManager) {
    ...
    public Packet handlePacket(Packet packetIn, ProxyServerConnection<P, T, C> connection) {
        ...
        // Returns a map of
        for (Map.Entry<String, ZulfProxyServer<P, T, C>> info : getProxy().getServersCopy().entrySet()) {
